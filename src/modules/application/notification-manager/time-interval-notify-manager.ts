...
export class TimeIntervalNotifyManager extends INotifyManager {
    ...
    async notify(): Promise<any> {
        ...
        const event = events.find(
            (e) => e.name == NotificationOptionEnum.timeInterval
        );
        ...
        if (event) {
            ...
            if (this.response.status == 3) {
                if (this.response.game_state == 3) {
                    ...
                    const lastInnings = events.find(
                        (e) => e.name == NotificationOptionEnum.lastInnings
                    );
                    if (!lastInnings) {
    ...
    private getCurrentTeam() {
        ...
        if (this.response.toss.decision == 1) {
            winnerTeam =
                this.response.teama.team_id == this.response.toss.winner
                    ? this.response.teama
                    : this.response.teamb;
        } else {
            winnerTeam =
        ...
        if (this.response.latest_inning_number == 1) {
            currentTeam =
                winnerTeam.team_id == this.response.teama.team_id
                    ? this.response.teama
                    : this.response.teamb;
        } else {
            currentTeam =
    ...
    private async triggerNotification(message: string, header: string) {
        const applications: string[] = this.match.applications.map<string>(
            (e) => e._id.toString()
        );
    ...
    private async updateMatchEvent(applications: string[]) {
        ...
        if (this.response.status == 2 || this.response.status == 4) {
            const filteredArray = events.filter(
                (item) => item.name != NotificationOptionEnum.timeInterval
            );
    ...
    private async updateEventTime() {
        ...
        try {
            ...
            return await this.liveMatchEventService.updateScheduleTmeFroInterval(
                this.match._id,
                targetTime
            );
        } catch (e) {}
