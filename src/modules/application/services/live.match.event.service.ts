...
export class LiveMatchEventService {
        ...
        try {
            ...
            if ((error as MongoError).code === 11000) {
                throw new ConflictException({
                    statusCode:
                        ENUM_APPLICATION_STATUS_CODE_ERROR.APPLICATION_EXIST_ERROR,
    ...
    async findAll<T>(
        ...
        const findAll = this.matchEventModel
            .find(find)
            .populate({
                path: 'applications',
    ...
    async findOneById<T>(_id: string): Promise<T> {
        const applications = this.matchEventModel
            .findById(_id)
            .populate({
                path: 'applications',
                model: ApplicationEntity.name,
    ...
    async update(_id: string, { applications, events }: LiveMatchUpdateDto) {
        const update = {
        ...
        const match: MatchEventDocument =
    ...
    async setNextEvents(match: MatchEventDocument) {
        ...
        if (events.includes(NotificationOptionEnum.toss)) {
            ...
            if (differenceInMs < 0) {
                ...
                console.log(
                    `The target time is ${differenceInMs} milliseconds from now.`
                );
                await this.updateScheduleTme(match._id, targetTime);
            }
        } else if (events.includes(NotificationOptionEnum.firstInnings)) {
            await this.updateScheduleTme(
                match._id,
                differenceInMs < 0 ? now : targetTime
            );
    ...
    async triggerEvents(match: MatchEventDocument) {
        ...
        try {
            ...
            if (
                events.map((e) => e.name).includes(NotificationOptionEnum.toss)
            ) {
                notifier = new TossNotifyManager(
                    resData,
                    this.triggerEvent,
                    match,
                    this
                ...
                events
                    .map((e) => e.name)
                    .includes(NotificationOptionEnum.firstInnings)
            ) {
                notifier = new FirstInningsNotifyManager(
                    resData,
                    this.triggerEvent,
                    match,
                    this
                ...
                events
                    .map((e) => e.name)
                    .includes(NotificationOptionEnum.lastInnings)
            ) {
                /// last innings started
                notifier = new LastInningsNotifyManager(
                    resData,
                    this.triggerEvent,
                    match,
                    this
            ...
            if (
                events
                    .map((e) => e.name)
                    .includes(NotificationOptionEnum.timeInterval)
            ) {
                notifier = new TimeIntervalNotifyManager(
                    resData,
                    this.triggerEvent,
                    match,
                    this
