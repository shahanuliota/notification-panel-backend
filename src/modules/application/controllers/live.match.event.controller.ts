...
export class LiveMatchEventController {
    ...
    @Post('add')
    saveMatch(
        @Body() dto: LiveMatchEventCreateDto,
    ...
    async getSavedLiveMach() {
        const list: MatchEventDocument[] =
            await this.liveMatchEventService.findAll<MatchEventDocument>({});
        return { data: list };
    }
    ...
    async updateLiveMatch(
        @GetLiveMatch() match: MatchEventDocument,
        @Body() dto: LiveMatchUpdateDto
    ) {
    ...
    async matchCornJob() {
        ...
        // console.log({find});
        const list: MatchEventDocument[] =
            await this.liveMatchEventService.findAll<MatchEventDocument>(find);

        for (const v of list) {
            await this.liveMatchEventService.triggerEvents(v);
        }

        const findSchedule: Record<string, any> = {
            scheduleForInterVal: {
                $gte: gt,
                $lte: lt,
            },
        };
        const scheduleList: MatchEventDocument[] =
            await this.liveMatchEventService.findAll<MatchEventDocument>(
                findSchedule
            );
        for (const v of scheduleList) {
            await this.liveMatchEventService.triggerIntervalEvents(v);
        }
        const remainingList: MatchEventDocument[] =
            await this.liveMatchEventService.findAll<MatchEventDocument>();
