...
export class ApplicationController {
    ...
    async create(
        ...
        const data1: ApplicationDocument = await this.applicationService.create(
            dto,
            user
        );
        return { ...data1 };
    ...
    @ResponsePaging('application.list', {
        classSerialization: ApplicationGetSerialization,
    })
    @UserProfileGuard()
    @AuthJwtGuard(ENUM_AUTH_PERMISSIONS.APPLICATION_READ)
    @Get('/list')
    async list(
        @Query()
        {
    ...
    async delete(
        @GetApplication() app: IApplicationDocument
    ): Promise<IResponse> {
        return await this.applicationService.deleteOne<IApplicationDocument>({
            _id: app._id,
        });
    }
    ...
    async getScheduledList(@GetUser() user: IUserDocument) {
        ...
        const tasks: TaskScheduleDocument[] =
            await this.taskScheduleService.findAll<TaskScheduleDocument>(find);
        const ojb = [];
            ...
            if (Array.isArray(applications)) {
                for (const app of applications) {
                    ...
                    const appl: ApplicationDocument =
                        await this.applicationService.findOne({
                            application_id: id,
                        });
                    if (appl) {
                        delete app.app_name;
                        ...
                        try {
                            ...
                            console.log({ v });
                            await this.taskScheduleService.deleteOne({
                                _id: v._id,
                            });
                        } catch (e) {
